...
trait ImageableTrait
{
    ...
    public function syncImagesByType($data, $type)
    {
        ...
        foreach (array_get($data, 'images.' . $type . '.path', []) as $key => $path) {
            if ($path) {
                ...
                $filename = array_get($data, 'images.' . $type . '.filename.' . $key, null);
                ...
                $description = array_get($data, 'images.' . $type . '.description.' . $key, '');
                $order = array_get($data, 'images.' . $type . '.order.' . $key, '');
    ...
    {
        return $this->mainImage->description ? $this->mainImage->description : $this->title;
    }
}

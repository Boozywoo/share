{!! Form::model($department, ['route' => ['admin.companies.set-department-bus', $department], 'class' => 'ibox-content form-horizontal js_form-ajax js_form-ajax-popup js_form-ajax-table js_form-current-page']) !!}
...
<div class="col-md-12">
                        ...
                        <tr>
                            <td>
                                <div class="checkbox m-n p-t-n">
                                    {{ Form::checkbox('buses['. $bus->id .']', $bus->id, $checked->search($bus->id), ['class' => 'js_checkbox', 'id' => 'buses['. $bus->id .']']) }}
                                    {{ Form::label('buses['. $bus->id .']', $bus->id) }}
                                </div>
                            </td>
                            <td>
        ...
        @else
            <p class="text-muted m-t-sm">Автобусы не найдены</p>
        @endif
    </div>
</div>
<div class="hr-line-dashed"></div>

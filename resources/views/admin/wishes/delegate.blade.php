...
@section('main')
    ...
    {!! Form::model($wishes,['route' => ['admin.'. $entity . '.delegate', $wishes], 'class' => "form-horizontal js_form-ajax js_form-ajax-redirect"])  !!}
            {!! Form::hidden('id') !!}
            <div class="ibox-content-custom">
                <div class="row">
                    <h4 style="text-align: center">{{ trans('admin.' .$entity . '.search_user') }}</h4>
                    <br>
                        {{ Form::panelSelect('delegate_id', $users, $wishes->delegate_id,  ['class'=>'js-user-delegate-single form-control']) }}

@if($orders->count())
    <div class="table-responsive">
        <h2>
            Начисления за брони
            @if($orders->where('status', 'active')->count())
                <a href="{{$urlExcel}}">
        ...
        <table class="table table-condensed">
            ...
            @foreach($orders->where('status', 'active') as $order)
                ...
                <tr>
                    ...
                    <td>
                    {{ $paysSum[$order->id]['sum'] . ' ' . trans('admin_labels.currencies_short.' . $paysSum[$order->id]['currency_alfa']) }}
        ...
        <table class="table table-condensed">
            ...
            <tr>
                <td>{{$orders->count()}}</td>
                <td>
                    @foreach($resultPrice as $alfa => $price)

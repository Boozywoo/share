...
(function ($) {
    ...
    TimeSlider.prototype.init = function(element, options) {
        ...
        this.$element.append(
            '<div class="ruler"  ondrop="drop(event)" ondragover="allowDrop(event)" style="height:' + (this.$element.height() + this.gt_height) + 'px;"></div>' +
    ...
    TimeSlider.prototype.add_cell = function(timecell) {
        ...
        if (! this.$ruler.find('#' + timecell['_id']).length) {
            ...
            this.$ruler.append(
                '<div class="qwerty">  '+
                '<div draggable="true"  onclick=alll()  ondragstart="drag(event)"  id="'+ timecell['_id'] +'" class="timecell'  + t_class + '" ' + start + ' ' + stop + ' style="' + style + timecell_style + '">' +
                this.time_duration(
                    (timecell['stop'] ? (timecell['stop']) : this.options.current_timestamp) - (timecell['start'])
                ) +
                '</div>' +
                '<div  onclick=alll()   id="t' + timecell['_id'] + '" p_id="' + timecell['_id'] + '" draggable="true" ondragstart="drag(event)" class="timecell-event'+ t_class + '" style="' + style + '"></div> </div>'
            );
            this.$prompts.append(
